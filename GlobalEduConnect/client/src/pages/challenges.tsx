...
import { ChallengeCardSkeleton } from "@/components/ui/skeleton";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function ChallengesPage() {
  ...
  // Get challenges
  const { data: challenges = [], isLoading: isChallengesLoadingRaw } = useQuery<Challenge[]>({
  ...
  const { data: myChallenges = [], isLoading: isUserChallengesLoadingRaw } = useQuery<Challenge[]>({
    queryKey: ["/api/challenges/my"],
    enabled: !!user,
  });
  
  // Use delayed loaders to prevent flickering for fast responses
  const isChallengesLoading = useDelayedLoader(isChallengesLoadingRaw, 300, 500);
  const isUserChallengesLoading = useDelayedLoader(isUserChallengesLoadingRaw, 300, 500);
          ...
          {isChallengesLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, index) => (
                <ChallengeCardSkeleton key={index} variant="shimmer" />
              ))}
            </div>
