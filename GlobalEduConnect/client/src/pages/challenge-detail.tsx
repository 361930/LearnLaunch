...
export default function ChallengeDetailPage() {
  ...
  const { data: challenge, isLoading: isChallengeLoadingRaw } = useQuery<Challenge>({
  ...
  const { data: participants = [], isLoading: isParticipantsLoadingRaw } = useQuery<ChallengeParticipation[]>({
    queryKey: [`/api/challenges/${challengeId}/participants`],
    enabled: !isNaN(challengeId),
  });

  // Fetch user's participation
  const { data: userParticipation, isLoading: isUserParticipationLoadingRaw } = useQuery<ChallengeParticipation>({
    queryKey: [`/api/challenges/${challengeId}/my-participation`],
    enabled: !isNaN(challengeId) && !!user,
  });

  // Fetch challenge comments
  const { data: comments = [], isLoading: isCommentsLoadingRaw } = useQuery<ChallengeComment[]>({
    queryKey: [`/api/challenges/${challengeId}/comments`],
    enabled: !isNaN(challengeId),
  });
  
  // Use delayed loaders to prevent flickering for fast responses
  const isChallengeLoading = useDelayedLoader(isChallengeLoadingRaw, 300, 500);
  const isParticipantsLoading = useDelayedLoader(isParticipantsLoadingRaw, 300, 500);
  const isUserParticipationLoading = useDelayedLoader(isUserParticipationLoadingRaw, 300, 500);
  const isCommentsLoading = useDelayedLoader(isCommentsLoadingRaw, 300, 500);
              ...
              <div className="md:col-span-2 space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                  <div className="w-full md:w-2/3">
                    <Skeleton className="h-10 w-3/4 mb-4" variant="shimmer" />
                ...
                <div>
                  <Skeleton className="h-40 w-full mb-6 rounded-lg" variant="shimmer" />
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    <div className="flex items-center">
                      <Skeleton className="h-8 w-8 rounded-full mr-2" variant="shimmer" />
                ...
                <div className="mt-6">
                  <Skeleton className="h-8 w-40 mb-4" variant="shimmer" />
                  <div className="space-y-4">
                    <Skeleton className="h-20 w-full rounded-lg" variant="shimmer" />
                    <Skeleton className="h-20 w-full rounded-lg" variant="shimmer" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="border rounded-lg p-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <Skeleton className="h-10 w-full rounded-md" variant="shimmer" />
                  </div>
                </div>
                ...
                <div className="border rounded-lg p-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <Skeleton className="h-10 w-full rounded-md mt-4" variant="shimmer" />
                  </div>
                </div>
              </div>
            </div>
